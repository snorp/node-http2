...
//
// - **http2.stripDeprecatedHeaderes(headers)**: strips deprecated headers from the given object
//
...
var DEPRECATED_HEADERS = exports.DEPRECATED_HEADERS = [
  'connection',
  'host',
  'keep-alive',
  'proxy-connection',
...
exports.stripDeprecatedHeaders = function(headers) {
  if (!headers) {
    return;
  }
...
IncomingMessage.prototype._validateHeaders = function _validateHeaders(headers) {
  ...
  //   PROTOCOL_ERROR.
  for (var i = 0; i < DEPRECATED_HEADERS.length; i++) {
    var key = DEPRECATED_HEADERS[i];
    if (key in headers) {
...
OutgoingMessage.prototype.setHeader = function setHeader(name, value) {
  if (this.headersSent) {
    ...
    if (DEPRECATED_HEADERS.indexOf(name) !== -1) {
...
function OutgoingResponse(stream) {
  ...
  this._onStateChange(this.stream.state);
  this.stream.on('state', this._onStateChange.bind(this));
...
OutgoingResponse.prototype._onStateChange = function onStateChange(state) {
...
Agent.prototype.request = function request(options, callback) {
  ...
  if (!options.plain && options.protocol === 'http:' && !this.host) {
  ...
  // * HTTP/2 over plain TCP
    ...
    endpoint.socket = net.connect({
      host: this.host || options.host,
      port: this.port || options.port,
      localAddress: this.localAddress || options.localAddress
    });
  ...
  // * HTTP/2 over TLS negotiated using NPN or ALPN
    ...
    var started = false;
    var httpsOptions = util._extend({}, options);
    httpsOptions.ALPNProtocols = supportedProtocols;
    httpsOptions.NPNProtocols = supportedProtocols;
    httpsOptions.servername = this.host || options.host; // Server Name Indication
    httpsOptions.host = this.host || options.host;
    httpsOptions.port = this.port || options.port;
    httpsOptions.agent = this._httpsAgent;
    httpsOptions.protocol = 'https:';
    httpsOptions.ciphers = options.ciphers || cipherSuites;

    var httpsRequest = https.request(httpsOptions);
